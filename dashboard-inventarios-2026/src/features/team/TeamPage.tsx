...
function getDayOptions(rows: InventarioRow[], selectedTeam: string): DayOption[] {
  const dates = new Map<string, string>();

  for (const row of rows) {
    if (!selectedTeam || row.team.trim() === selectedTeam) {
...
export function TeamPage() {
  ...
  const allTeams = useMemo(() => getAvailableTeams(rows), [rows]);
  ...
  const users = useMemo(() => getAvailableUsers(dateRows.filter((row) => (selectedTeam ? row.team.trim() === selectedTeam : true))), [dateRows, selectedTeam]);
  ...
  const monthlyStats = useMemo(() => getMonthlyReachedStats(groupByTeamUserDay(filteredRows)), [filteredRows]);

  const tableRows = useMemo(() => {
    const dayRows = selectedDay ? filteredRows.filter((row) => row.fecha === selectedDay) : filteredRows;
    const metrics = groupByTeamUserDay(dayRows);
  ...
  if (allTeams.length === 0) {
    return <EmptyState title="Sin teams" message="No hay teams disponibles en las filas validas." />;
  }
  ...
  return (
    <section className="page">
      <div className="page__header">
        <div>
          <span className="page__eyebrow">Vista por team</span>
          <h2 className="page__title">Resumen diario filtrado por team</h2>
        </div>
      </div>
      ...
      <div className="kpi-grid">
        ...
        <KpiCard label="Usuarios" value={formatNumber(new Set(tableRows.map((metric) => metric.correo)).size)} />
      ...
      <DataTable
        ...
        emptyMessage="No hay metricas para este team y filtros seleccionados."
      />
    </section>
  );
}
